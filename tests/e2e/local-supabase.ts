...
export async function deleteLocalTestUsers(userIds: string[]) {
  ...
  execFileSync("docker",["exec","supabase_db_helm-project-command-center","psql","-U","postgres","-d","postgres","-v","ON_ERROR_STOP=1","-c",`delete from public.project_change_requests where project_id in (select p.id from public.projects p join public.organizations o on o.id=p.organization_id where o.created_by in (${ids}));delete from public.project_status_reports where project_id in (select p.id from public.projects p join public.organizations o on o.id=p.organization_id where o.created_by in (${ids}));`],{stdio:"ignore"});
...
export function provisionLocalProjectMember(input:{userId:string;email:string;fullName:string;organizationId:string;projectId?:string;accessLevel?:"Project Manager"|"Project Member"|"Stakeholder"|"Read Only"}){
  ...
  const accessLevel=input.accessLevel??"Project Member";const projectMembership=input.projectId?`insert into public.project_members(project_id,user_id,project_role,access_level) values ('${input.projectId}','${input.userId}','Delivery team','${accessLevel}');`:"";
...
export function deactivateLocalOrganizationMember(userId:string,organizationId:string){const uuidPattern=/^[0-9a-f-]{36}$/i;if(!uuidPattern.test(userId)||!uuidPattern.test(organizationId))throw new Error("Refusing an invalid local membership update.");const sql=`update public.organization_members set status='Inactive' where user_id='${userId}' and organization_id='${organizationId}';`;execFileSync("docker",["exec","supabase_db_helm-project-command-center","psql","-U","postgres","-d","postgres","-v","ON_ERROR_STOP=1","-c",sql],{stdio:"ignore"});}
export function assignLocalProjectManager(userId:string,projectId:string){const uuidPattern=/^[0-9a-f-]{36}$/i;if(!uuidPattern.test(userId)||!uuidPattern.test(projectId))throw new Error("Refusing an invalid local manager assignment.");execFileSync("docker",["exec","supabase_db_helm-project-command-center","psql","-U","postgres","-d","postgres","-v","ON_ERROR_STOP=1","-c",`set session_replication_role=replica; update public.projects set project_manager_id='${userId}' where id='${projectId}'; set session_replication_role=origin;`],{stdio:"ignore"});}
