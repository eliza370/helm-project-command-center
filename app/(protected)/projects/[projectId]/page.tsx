...
export default async function ProjectPage({params,searchParams}:{params:Promise<{projectId:string}>;searchParams:Promise<{created?:string;updated?:string}>}) {
  ...
  return <ApplicationShell organizationName={membership.membership.organizationName} userLabel={profile?.full_name||data.user.email||"Signed-in user"}><section className="mx-auto max-w-6xl px-5 py-10 sm:px-8">
    <Link href={`/projects/${project.id}/status-reports`} className="mb-4 inline-block rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800">Status Reports</Link>
    ...
    <div className="flex flex-wrap items-center justify-between gap-4"><Link href="/projects" className="text-sm font-semibold text-slate-700 underline underline-offset-4">Back to projects</Link><div className="flex flex-wrap gap-3"><Link href={`/projects/${project.id}/decisions`} className="rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800">Decision Log</Link><Link href={`/projects/${project.id}/raid`} className="rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800">RAID register</Link><Link href={`/projects/${project.id}/actions`} className="rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800">Actions</Link><Link href={`/projects/${project.id}/deliverables`} className="rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800">Deliverables</Link><Link href={`/projects/${project.id}/milestones`} className="rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800">Milestones</Link><Link href={`/projects/${project.id}/team`} className="rounded-lg border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800">Project team</Link>{editable?<Link href={`/projects/${project.id}/edit`} className="rounded-lg bg-slate-900 px-5 py-3 text-sm font-semibold text-white">Edit project</Link>:null}</div></div>
